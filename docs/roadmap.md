...
- **`DenseRetriever` sigue expuesto al mismo tipo de falla que se arregló en el reranker** (surgió arreglando el bug de rate limit de la Etapa 10, después de la Etapa 13). `HybridRetriever` necesita `embed_query()` para la mitad densa — si ese llamado a Voyage tira `RateLimitError`, hoy no hay fallback y se propaga igual que le pasaba antes al reranker. La solución no es tan directa como la del reranker (ahí "no rerankear" es un fallback obvio; acá haría falta que `HybridRetriever` caiga a devolver solo la mitad léxica, o que `DenseRetriever` devuelva vacío y deje que RRF opere solo con lo que tiene). Identificado pero no resuelto — queda pendiente si se repite en la práctica.
