...
> por completo por este roadmap, las ADRs y
> `docs/applied-ai-interview-prep.md`), incorporando las correcciones
...
- **`GitHubConnector` solo indexa Markdown (`.md`/`.markdown`), nunca código fuente** (surgió en la Etapa 14, decisión deliberada, pendiente de revisar). Mezclar código junto con notas/documentación en el mismo índice podría agregar ruido de retrieval en vez de valor — un chunk de código compite por los mismos top-k que un chunk de prosa, con una noción de "relevancia" semánticamente distinta, y `HeadingChunker` parte por encabezados Markdown, así que código fuente necesitaría su propia estrategia de chunking (por función/clase) para no quedar cortado sin sentido. Pospuesto a propósito hasta tener más criterio de RAG para evaluar el trade-off, no por costo de implementación (extender `_MARKDOWN_SUFFIXES` en `sources/connectors/github.py` es trivial; el chunker dedicado para código es la parte real).
...
- ~~**Documento educacional pendiente, a armar recién cuando el roadmap esté completo**~~ (2026-08, pedido explícito; **resuelto en 2026-08**) — `docs/learning-notes.md` (el borrador acumulativo) se consolidó en `docs/applied-ai-interview-prep.md`: organizado por concepto en vez de por etapa, con cada bug/incidente real conservado (son las mejores respuestas a "contame de un bug difícil que resolviste"), y preguntas tipo por sección para practicar recuperar la info de memoria. `learning-notes.md` se eliminó del repo, reemplazado por completo.
...
- **Los modales quedaban tapados por el chrome** (2026-08, reportado con screenshot). El encabezado sticky del corpus se pintaba **encima** del diálogo de System Status. Causa medida, no adivinada: **los dos diálogos no tenían `z-index` en absoluto**, mientras el sticky tenía 2 y el drawer 20–30. El arreglo no fue subir números sueltos sino definir una **escala en tokens** (`--z-sticky`, `--z-drawer`, `--z-scrim`, `--z-modal`) y migrar los once usos: cualquier cosa que se superponga elige un escalón en vez de inventar un número. Era exactamente la clase de bug que reaparece si se parchea caso por caso.
- **Scrollbars** (2026-08, reportado con screenshot: cortaban el texto de los mensajes). La barra opaca del sistema vive dentro del padding box y se comía el borde derecho de la prosa. Ahora el pulgar flota sobre el contenido (`background-clip: content-box` con borde transparente, que además mantiene el área de agarre más ancha que la parte visible) y solo aparece al pasar el mouse. Los contenedores que scrollean llevan `scrollbar-gutter: stable`, así el texto no se reacomoda cuando la barra aparece.
- **Auto-scroll de la conversación** (2026-08, a pedido): al enviar y **otra vez cuando la respuesta llega**, porque la respuesta cambia la altura después del hecho — scrollear solo al enviar dejaba la respuesta debajo del pliegue. Va dentro de `requestAnimationFrame` porque leer `scrollHeight` en el mismo tick devuelve la altura vieja. Abrir una conversación existente también arranca en el turno más nuevo, pero con salto instantáneo en vez de suave: es la posición inicial, no un movimiento que el lector deba mirar.
- **Botón "Sync now"** (2026-08, a pedido): encolar una sync vuelve casi instantáneo (el trabajo ocurre en un worker), así que el botón parpadeaba de deshabilitado a habilitado y se leía como "no pasó nada". Ahora hay un piso de 1200ms más un spinner, y queda deshabilitado también mientras la fuente está en `syncing`. El retraso es honestidad sobre una acción cuyo efecto no es visible de inmediato, no decoración.
- **Íconos de tema** (2026-08, a pedido): sol / luna / monitor en vez del texto "Theme: System". El de "system" es un monitor a propósito — significa "lo que diga la máquina", no un tercer color.
- **El picker de carpetas: no estaba roto, la expectativa era imposible** (2026-08). El usuario esperaba que abriera Finder. **Ningún navegador puede hacer eso**: no existe API que le dé al servidor una ruta del host — `showDirectoryPicker()` da acceso de lectura *en la página* y `<input webkitdirectory>` da contenidos para subir, ninguna da una ruta usable por el backend. Y Lorebase indexa desde el filesystem *del servidor*. Verificado en vivo: el picker listaba correctamente `/app/storage`, pero el `docker-compose.yml` solo montaba `../backend`, así que el contenedor **no podía ver nada de la Mac**. El arreglo real es de infraestructura: un bind mount opcional de solo lectura (`LOREBASE_NOTES_DIR`) que expone una carpeta del host en `/app/storage/external`, documentado en `.env.example`. Además el modal ahora **dice qué está mostrando**, porque la suposición obvia (que es tu disco) es la equivocada.
- **El panel de System Status** (2026-08, a pedido: "está medio escondido y le faltaría más información"). Pasó de un link perdido en el pie de la sidebar a un destino primario junto a Corpus, y abre con una sección nueva **"Answer quality" arriba de todo**: respuestas, sin fundamentar (con porcentaje, en rojo si hay alguna), fuentes citadas por respuesta y latencia media. El orden es deliberado: la métrica que dice si el sistema está haciendo su trabajo va primero, no los conteos de contenido.
- **El botón "Add source" parecía haber desaparecido** (2026-08, reportado por el usuario). No se había borrado: vivía al final de `.sources`, que tiene `max-height: 45%` con scroll, así que con cinco fuentes quedaba **debajo del pliegue dentro de ese contenedor**. Una acción primaria escondida en un scroll es indistinguible de una acción que no existe. Se movió al encabezado de la sección como un `+` que aparece al hacer hover — el patrón de Linear — donde no puede irse de la vista sin importar cuánto crezca la lista.
- **El botón de System Status se veía como una caja suelta** (2026-08, reportado con screenshot). Dos causas: se lo había puesto como ítem de navegación entre dos grupos, donde no pertenecía a nada, y **el reset de `border`/`background` que creí haberle aplicado nunca llegó al archivo**, así que renderizaba con el chrome por defecto del navegador. Ahora vive en el pie junto al tema y el cierre de sesión, como una fila de íconos discretos. Mismo destino para el control de tema: el ícono suelto al lado de links subrayados no funcionaba; ahora los tres son botones de ícono del mismo tamaño.
- **Token semántico `--success`** (2026-08, a pedido: "los círculos podrían ser verdes para que se note que el proceso va bien"). No existía: el acento es tinta, así que "este paso terminó bien" no tenía con qué decirse y usaba un punto negro relleno, que no dice nada sobre el resultado. Es color semántico, separado del acento, y solo se usa para estado completado y lecturas sanas — nunca para un control. Contraste verificado en ambos temas.
- **El alta de una fuente ahora muestra progreso real** (2026-08, a pedido): los pasos se conectan con un riel para que se lean como un proceso y no como tres viñetas sueltas, el paso en curso pulsa, los completados se marcan en verde con un tilde, y **debajo aparece una barra con los números reales** (`N de M chunks embebidos`) apenas existen. Un spinner solo dice "esperá"; los números dicen cuánto falta.
- **Sexto componente extraído por el presupuesto de estilos** (`SourceListComponent`). Antes de extraer se buscó CSS muerto y no había — o sea que el componente genuinamente había crecido de más, que es justo lo que el presupuesto está para detectar. **Error propio en el camino:** el script que quitaba los métodos que se mudaban usó un regex con `.*?` sobre múltiples líneas y se llevó también `ngOnInit`, `anyInFlight` y el manejo del media query. Lo detectó el linter (cuatro variables declaradas sin usar); se restauró el archivo desde git y se rehicieron los cambios de a uno. Vale como recordatorio: para borrar bloques de código, un regex codicioso es peor herramienta que un reemplazo de texto exacto.
- **`Source.enabled`: indexado pero no buscado** (2026-08, a pedido). Un booleano, no un soft delete: los chunks y embeddings quedan intactos, así que volver a habilitar es instantáneo y no re-indexa nada, y **las citas de respuestas viejas siguen resolviendo** porque los chunks que apuntan siguen existiendo. La exclusión vive en `apply_filters()`, que es el punto de paso obligado de los tres retrievers, y se aplica **antes del early return**: "una fuente apagada nunca se busca" es un invariante como el scoping por workspace, no una preferencia del que llama. Ponerlo detrás del `if filters` habría hecho que cualquier caller que pasa `None` buscara en silencio en fuentes apagadas — cubierto con un test que fija exactamente eso. Verificado en vivo: apagar `root` baja de 2802 a 1839 chunks buscables y volver a encenderlo los devuelve.
- **Los puntos de estado habían dejado de decir nada** (2026-08, reportado por el usuario). Usaban `var(--accent)`, que **era verde cuando se escribieron**; al cambiar el acento a tinta se volvieron negros y dejaron de comunicar "sano" sin que nada fallara — el estado seguía siendo correcto, el color dejó de expresarlo. Ahora usan `--success`. Es un buen ejemplo de por qué los tokens semánticos importan: `--accent` describía *dónde* se usaba el color, no *qué significaba*, así que cambiarlo rompió un significado sin tocar ninguna lógica.
- **Errores del chat, distinguidos** (2026-08, a pedido). Antes cualquier fallo del proveedor salía como un 500 sin detalle. Ahora `LLMProviderUnavailableError` (mismo patrón que reranker y embeddings) traduce rate limit, timeout, corte de conexión y error de upstream, y el view responde **503 con "try again"** para eso y **500 sin invitación a reintentar** para un bug — porque mandar a alguien a reintentar un bug es mandarlo a un loop que no puede terminar. `InvalidRequestError` queda deliberadamente afuera de "no disponible": una request malformada es un error nuestro. También se cubrió el caso de que el modelo devuelva una respuesta sin el bloque `tool_use` que `tool_choice` fuerza: antes reventaba con `StopIteration`, que no le decía nada a nadie.
- **Las citas pasaron de columna al margen a fila bajo la respuesta** (2026-08, a pedido: "me resulta rara"). El diagnóstico coincide con el del usuario: la marginalia es una convención del libro anotado, y **el contenido que estábamos poniendo ahí son rutas y números**, no glosa. Además reservaba 190px que quedaban casi siempre vacíos y le quitaban ancho a la respuesta. Ahora son chips numerados en una fila, con la etiqueta "Sources" — el patrón que ya se lee como "esto es sobre lo que se apoya" — y el pasaje sigue abriéndose en el lector anclado. La respuesta recupera todo el ancho de lectura.
- **El pasaje citado se abre en línea, y el panel anclado se eliminó** (2026-08, a pedido de una alternativa "más natural e integrada"). Vale registrar por qué el panel existió: era **la respuesta correcta al layout anterior**, donde el control vivía en una nota al margen y no había ningún lugar adyacente sensato donde expandir. Al mover las citas a una fila al pie de la respuesta, la adyacencia quedó disponible y el panel dejó de justificar su costo — una superficie aparte, con scrim, escape y foco propio, para mostrar un bloque de texto. Ahora el pasaje se abre justo debajo del chip que lo abrió, en la misma columna, sin nada que descartar. `PassageReaderComponent` se borró. **La lección es sobre el orden**: la solución no estaba mal, el problema que resolvía había dejado de existir.
- **Bug de fondo encontrado en el camino: los estilos del Markdown de las respuestas nunca habían aplicado** (2026-08). La encapsulación emulada de Angular reescribe `.rendered-markdown p` como `.rendered-markdown[_ngcontent-x] p[_ngcontent-x]`, y **el HTML inyectado por `[innerHTML]` no lleva ese atributo**. Verificado en el bundle servido: las reglas salían compiladas con `_ngcontent-%COMP%` en el descendiente, así que ningún párrafo, lista, encabezado o bloque de código de una respuesta estuvo nunca estilado — se veían con los defaults del navegador. Se movieron al stylesheet global, que es donde tienen que estar: los estilos de markup inyectado no pueden vivir dentro del scope de un componente. Verificado después del fix: ahora se compilan sin `_ngcontent`.
- **El skeleton de carga nunca había llegado al template** (2026-08). El CSS se commiteó hace muchas ediciones; el markup no, porque el `replace` que lo insertaba no matcheó y el script siguió sin avisar. Lo detectó un chequeo de clases CSS definidas y no usadas en el HTML, hecho para otra cosa. **Es el tercer caso del mismo patrón en esta sesión** (el `<header class="query">`, los estilos de `.corpus-link`, y este): un reemplazo exacto que falla en silencio y deja el CSS sin su markup. La contramedida adoptada es editar por líneas verificando el resultado, y correr el chequeo de huérfanas después de tocar plantillas.
- **Borrado de fuentes movido al Corpus** (2026-08, a pedido), que es la pantalla donde se ve qué se perdería. Y el texto de confirmación dejó de mentir por omisión: antes decía "your original files are not touched" — cierto sobre los archivos, engañoso sobre todo lo demás. Ahora nombra los chunks que se van, avisa que **las respuestas que la citaron conservan el texto pero pierden esas citas**, y ofrece la alternativa: apagarla en vez de borrarla.
- **El estado de la fuente se muestra en el sidebar** (2026-08, a pedido): nombre y etiqueta atenuados cuando está apagada, punto en gris, y la etiqueta dice "Not used for answers" — el único estado que merece palabras, porque es una decisión de alguien y no una transición, y nada más en la fila explica por qué está grisada.
- **Bug grave y auto-infligido: las respuestas se mostraban con estilo de error** (2026-08, reportado con screenshot). Al mover las reglas de Markdown al stylesheet global (ver arriba), el script usó una segunda pasada de regex con `re.S` que **fusionó selectores de reglas distintas**, dejando `.rendered-markdown p, ul, ol, pre, h1, h2, .load-error { color: danger; background: danger-soft; ... }`. Resultado: **todo párrafo, lista y encabezado de una respuesta heredó el fondo rosa y el texto rojo del mensaje de error**, con 5 citas válidas. Además la extracción partió reglas multi-selector (`blockquote` terminó definido tres veces, `p` y `ul` perdidos) y se llevó `.load-error` del componente. Se reescribió el bloque entero a mano y se verificó en el bundle **servido** que las reglas salen correctas y sin contaminación. **Es el cuarto incidente de la misma familia en esta sesión**, y el más caro: los tres anteriores dejaron CSS sin markup (fallo silencioso, cosmético); este produjo CSS *incorrecto* que sí aplicaba. La conclusión operativa quedó clara: **no editar CSS con regex sobre bloques**. Reescribir la sección completa a mano y verificar contando llaves y listando selectores es más lento y no falla así.
- **Los botones de acción de una fuente quedaron con un solo estilo** (2026-08, a pedido). "Use for answers", "Delete" y "Sync now" hacen cosas comparables y estaban con tres pesos visuales distintos; ninguno se gana una jerarquía sobre los otros. Delete difiere únicamente en hover, que es cuando la consecuencia está por ocurrir.
- **El drawer del sidebar en pantallas angostas dejó de oscurecer la página** (2026-08, a pedido). El scrim quedó transparente: existe solo para capturar un tap fuera del panel. Oscurecer implicaba que el contenido de atrás estaba bloqueado, y nunca lo estuvo — es un panel de navegación, no un modal.
- **Una sola regla por turno en el chat** (2026-08, a pedido: "la línea que separa la pregunta de la caja"). Había dos: una arriba de la pregunta y otra bajo la fila de metadatos, así que cada intercambio llegaba encajonado entre dos líneas. La fila de metadatos ya está en mono contra prosa en serif, y ese cambio de registro hace el trabajo solo.
- **"Falta el botón para agregar Sources", por segunda vez** (2026-08). La primera fue porque estaba al final de una lista con scroll y quedaba bajo el pliegue; se movió al encabezado de la sección. La segunda fue por lo que se hizo al moverlo: se copió el patrón de Linear de **revelar la acción con hover** (`opacity: 0` en reposo). Ahí está la diferencia que no vi: en Linear esas acciones son afordancias secundarias sobre filas que ya conocés, mientras que **agregar una fuente es una de las dos cosas para las que existe esta app** y lo primero que necesita alguien con la sidebar vacía. Una acción primaria invisible se reporta como una acción faltante — literalmente pasó dos veces. Ahora está siempre visible, con borde propio, y el estado vacío pasó de ser una frase que *describe* la acción a ser el botón que la ejecuta.
  - Se auditó el resto de la app buscando el mismo patrón. El botón de sync de una fila también estaba oculto tras hover y **solo se renderiza para fuentes en `pending`/`error`**, o sea justo cuando algo hay que hacer: esconder el remedio de un estado que pide atención anula el sentido de mostrar el estado. También se hizo permanente. Los dos casos restantes se dejaron a propósito: el tilde dentro del círculo de progreso es un indicador de estado, no una acción, y borrar una conversación sí es secundaria sobre una fila ya visible (y queda alcanzable por teclado con `:focus-within`).
- **Tercera vez que "falta el botón de agregar Sources", tercera causa distinta** (2026-08). Esta fue la más tonta y la más difícil de ver mirando la pantalla: `.section-label` tenía `width: 100%` **junto con** `flex-shrink: 0` dentro de un flex row, así que se quedaba con todo el ancho y se negaba a devolver nada — el `+` hermano quedaba empujado fuera de `.section-head` y **recortado por el `overflow: hidden` del rail**. El botón se estaba renderizando todo el tiempo, fuera del borde del panel. Confirmado en el bundle servido (el `title="Add source"` estaba presente) antes de tocar CSS. `flex: 1; min-width: 0` lo resuelve. Las tres causas fueron: quedar bajo el pliegue de un scroll, quedar invisible por `opacity: 0`, y quedar fuera del contenedor por un ancho rígido — vale como recordatorio de que "no está" casi nunca significa "no se renderiza".
- **Limpieza de código muerto** (2026-08, a pedido). Se buscó de forma sistemática en vez de a ojo: clases CSS definidas sin usar en su template, tokens definidos sin referencias, componentes y servicios TypeScript sin importar, métodos públicos sin llamadas, y símbolos de Python sin referencias fuera de su archivo. **Casi todo dio limpio** — los aparentes huérfanos de Python eran falsos positivos (registrados por decorador de Django o referenciados por string en `urls.py`), y los de CSS eran clases múltiples (`class="pane documents"`) que el regex no reconocía. Lo realmente muerto: las tres reglas de `.sync-btn` (reemplazadas por `.row-btn`) y la dependencia **`einops`**, que había quedado declarada tras el pivot de Jina y no la importa ninguna línea del proyecto — solo el código cacheado del modelo abandonado.
  - **Verificar el borrado de `einops` costó más que hacerlo**, y expuso de nuevo el mismo tropiezo de Docker: el venv vive en un volumen nombrado, así que rebuildear la imagen **no lo actualiza** y el paquete seguía instalado. Al recrear el volumen apareció una carrera nueva: `backend` y `worker` montan el mismo volumen y ambos intentan poblarlo desde la imagen a la vez, lo que falla con `file exists`. La solución es inicializarlo con **un solo servicio primero** (`up -d db redis`, después `up -d backend`, después el resto). Verificado al final: `einops` ausente, reranker y embeddings funcionando, y los 2805 chunks con sus embeddings intactos.
- **Los `.env` documentaban menos de la mitad de las variables configurables** (2026-08, a pedido). Se auditó extrayendo del código cada llamada a `env(...)` en los settings más cada `${VAR}` de `docker-compose.yml`, y comparando contra los archivos: **27 variables reales, 15 documentadas en `infra/.env`, 9 en `backend/.env`**. Las 12 faltantes eran todas opcionales (tienen default), pero eso es justamente el problema: nadie podía saber que existían sin leer `config/settings/base.py` — entre ellas `RETRIEVAL_STRATEGY`, `LLM_MODEL`, `EMBEDDING_DIMENSIONS` y los tres de costo. `.env.example` se reescribió entero y agrupado, con **cada variable comentada mostrando su default**, de modo que copiar el archivo tal cual da una configuración que funciona y cada línea descomentada es un override deliberado. Verificado programáticamente: cero faltantes y cero sobrantes contra el set del código.
  - En los dos `.env` reales (ignorados por git, con claves de verdad) **solo se agregaron líneas comentadas**, nunca se tocó un valor activo — confirmado con un diff contra una copia previa antes de borrarla.
  - `backend/.env` deja fuera a propósito `POSTGRES_*` y `LOREBASE_NOTES_DIR`: las lee únicamente `docker-compose.yml` para configurar los contenedores, así que fuera de Docker no hacen nada. Quedó anotado en el archivo para que la ausencia se lea como decisión y no como olvido.
...
- **Cancelar un sync en curso** (2026-08, a pedido). No existía forma de parar un sync ya encolado — con el connector de GitHub, donde cada archivo cuesta 2 requests HTTP, un repo grande podía tardar minutos sin salida. Cancelación cooperativa, no un kill: matar la task de Celery a la fuerza podría interrumpir un documento a mitad de escribirse. `sources/locking.py` suma un flag por source en el mismo cache de Redis que ya usa `sync_lock` (`request_cancel`/`is_cancel_requested`/`clear_cancel`); `sync_source()` lo chequea una vez por documento, en el punto seguro entre uno y el siguiente. Si cancela, **se saltea por completo el paso que marca documentos como borrados** — `seen_external_ids` en ese punto es un listado parcial, y tratar todo lo no visto como "ya no está" hubiera borrado documentos reales que el sync nunca llegó a mirar. Lo ya ingerido antes de cancelar queda (sync es incremental por `content_hash`, así que retomar más tarde no repite trabajo). Nuevo estado `SyncRun.Status.CANCELLED`; `Source.status` vuelve a `ready` igual que un sync exitoso, porque lo parcial ingerido es real y consultable. El flag se limpia defensivamente al *empezar* cada sync (no solo al consumirlo), para que uno viejo sin limpiar no cancele un sync distinto más adelante. Endpoint `POST /api/sources/{id}/cancel_sync/`, rechazado con 409 si el source no está `syncing` en ese momento — así un click que llega justo después de que el sync ya terminó no deja un flag colgado. Botón "Stop sync" en Corpus, reemplazando "Sync now" mientras `syncing()` es true.
...
| 48 | Escala de z-index; modales tapados por el chrome | Bug propio | Etapa 13 |
| 49 | Scrollbars, auto-scroll y botón de sync | Pedido | Etapa 13 |
| 50 | Carpetas del host montadas para el picker | Pedido | Etapa 5 y 13 |
| 51 | "Add source" escondido dentro de un scroll | Bug propio | Etapa 13 |
| 52 | Sidebar más denso; progreso de alta con color semántico | Pedido | Etapa 13 |
| 53 | Habilitar/deshabilitar un source para el retrieval | Pedido | Etapa 10 |
| 54 | Errores del chat distinguidos y accionables | Pedido | Etapa 11 |
| 55 | Las citas pasan a una fila bajo la respuesta | Pedido | Etapa 13 |
| 56 | El pasaje se abre en línea; se elimina el panel anclado | Pedido | Etapa 13 |
| 57 | Los estilos de Markdown nunca habían aplicado | Bug propio | Etapa 13 |
| 58 | Regex de CSS fusionó selectores y pintó las respuestas de rojo | Bug propio | Etapa 13 |
| 59 | Acciones primarias ocultas tras hover | Bug propio | Etapa 13 |
...
| 66 | Cancelar un sync en curso | Pedido | Etapa 7 |
...
- **Bug real encontrado al reconstruir la imagen de Docker para sumar las dependencias de OTel**: `torch` (traído transitivamente por `sentence-transformers`, para `LocalReranker`) resolvía contra PyPI normal —con CUDA completo y ~2GB de paquetes `nvidia-*`/`triton`— pese a que `pyproject.toml` ya declaraba un índice CPU-only (`pytorch-cpu`) para `torch`. Causa: `tool.uv.sources` de `uv` solo reescribe el índice de dependencias **directas**; como `torch` nunca estaba listado en `[project.dependencies]`, la restricción no se aplicaba. Corregido declarándolo como dependencia directa. De paso se descubrió que lo que realmente llenaba el disco de la VM de Docker Desktop no eran las imágenes viejas sino **41GB de build cache de BuildKit** (32GB reclamables) acumulados por rebuilds sucesivos bajando esos mismos paquetes pesados una y otra vez — podado con `docker builder prune`.
- **Verificación en vivo evitó confiar en el flush asíncrono del `BatchSpanProcessor`**: en vez de solo mandar una pregunta real por el chat y asumir que el span llegó a Langfuse, se capturó un span con `InMemorySpanExporter` y se lo pasó directo a `OTLPSpanExporter().export(...)`, confirmando `SpanExportResult.SUCCESS` (auth + endpoint correctos) de forma síncrona y sin ambigüedad.
- **Golden set de 30 preguntas (Tarea 3) construido sobre `career_notes.md`/`career_notes_es.md`** (el CV/historial técnico real del usuario, en inglés y español), no sobre notas sintéticas: da contenido fácticamente denso y verificable, y las versiones EN/ES duplicadas permiten probar retrieval cross-lingual a propósito. `expected_document` es una lista (no un string), justamente para no penalizar un retrieval correcto que acierta la versión en el otro idioma. Cada una de las 30 entradas se validó por código contra la base real (`expected_document` + `expected_heading` resuelven a un chunk indexado existente), no solo a ojo.
- **`ragas==0.4.3` (la versión más nueva al momento) tiene un import roto** (`langchain_community.chat_models.vertexai`, movido a otro paquete río arriba, issues [#2741](https://github.com/vibrantlabsai/ragas/issues/2741)/[#2745](https://github.com/vibrantlabsai/ragas/issues/2745) sin fix) — se fijó `ragas==0.3.9` más `langchain-community<0.4` explícito.
- **La API "moderna" que sugiere el propio warning de deprecación de `LangchainLLMWrapper` (`llm_factory`) no sirve para este caso**: devuelve `InstructorBaseRagasLLM`, que no hereda de `BaseRagasLLM` — la interfaz que `Faithfulness`/`LLMContextRecall`/etc. requieren. Verificado con `issubclass()` contra el paquete instalado antes de descartar la opción. `LangchainLLMWrapper` se sigue usando a propósito, importado desde `ragas.llms.base` (no desde `ragas.llms`, cuyo top-level lo redirige a través de un proxy `DeprecationHelper` que ni siquiera sirve para `isinstance`).
- **`import ragas` requiere el binario `git` en el sistema** (GitPython, para una función de tracking de experimentos que no se usa), y la imagen de runtime de Docker no lo tiene a propósito. Resuelto con `os.environ.setdefault("GIT_PYTHON_REFRESH", "quiet")` antes del import, en vez de instalar `git` en la imagen para una dependencia transitiva de una feature no usada.
- **Juez de RAGAS: mismo proveedor (Anthropic) y modelo que ya genera las respuestas** — una sola API key, más barato, con el trade-off nombrado explícitamente de que un modelo juzgando a su propia familia tiene sesgo de auto-preferencia documentado en la literatura de evaluación.
- **Smoke test en vivo (no solo unitario) contra el golden set real**: pregunta sobre el bug de concurrencia de `cycle automation`, con juez real. Resultado: `context_precision=1.0`, `context_recall=1.0`, `faithfulness=1.0`, `answer_relevancy=0.89` — confirma la cadena completa (retrieval → LLM → RAGAS) funcionando, no solo que el código compila.
- **`manage.py evaluate` (Tarea 5) necesitó exponer los chunks recuperados desde `ask()`, no solo lo citado**: `retrieved_contexts` para RAGAS (y el chequeo determinístico de hit-rate) tiene que ser *todo* lo que el retriever le pasó al LLM como contexto, no el subconjunto que terminó citado — usar `message.citations` (como hacía el smoke test manual original) habría inflado artificialmente `context_precision`. Se refactorizó `ask()` en `_ask()` (devuelve `Message` + `list[RetrievalResult]`) con dos wrappers finos: `ask()` (sin cambios para los llamadores existentes) y `ask_with_contexts()` (para el harness de evaluación).
...
**Notas de la implementación real (Tareas 1 y 2):**
- **CORS (Tarea 1) deliberadamente no se agregó**, pese a que el roadmap lo nombra explícitamente: `frontend/nginx.conf` ya proxea `/api/*` same-origin (es lo que hace andar la cookie de sesión y CSRF sin ninguna config cross-origin) — no hay ningún pedido cross-origin en esta arquitectura de deploy para que headers de CORS gobiernen. Verificado leyendo el nginx.conf real, no asumido.
- **`manage.py check --deploy` (el chequeo propio de Django) se usó para verificar el hardening**, en vez de confiar en la lista de settings de memoria — 0 issues reales tras agregar `SECURE_PROXY_SSL_HEADER`, HSTS, `SECURE_CONTENT_TYPE_NOSNIFF`, `SECURE_REFERRER_POLICY` (el único warning que queda es sobre una `SECRET_KEY` de prueba usada solo para correr el check).
- **Logging estructurado (JSON) implementado con la librería estándar** (`config/logging.py`, un `logging.Formatter` de ~15 líneas), no una dependencia nueva — no hacía falta más para este alcance.
- **Incidente real durante la verificación de Tarea 2, con causa y arreglo**: `docker-compose.prod.yml` no tenía nombre de proyecto propio, y al vivir en la misma carpeta `infra/` que el compose de dev, Docker Compose infiere el mismo namespace para los dos — el primer `docker compose ... up` con el archivo de prod **recreó los contenedores del stack de dev** y estuvo a punto de compartir el volumen real de Postgres (`pgdata`). No hubo pérdida de datos (Postgres solo inicializa un volumen vacío; uno ya poblado con datos reales no se toca al recrear el contenedor con credenciales distintas, simplemente falla el login) — verificado explícitamente releyendo `Workspace`/`Document` después. Arreglado con `name: lorebase-prod` más volúmenes `pgdata_prod`/`redisdata_prod` explícitamente separados, para que la colisión sea estructuralmente imposible, no solo evitada por convención.
- **El healthcheck de `backend` heredado de dev rompe en prod**: con `SECURE_SSL_REDIRECT=True`, un pedido HTTP plano (lo que hace el healthcheck, corriendo dentro del propio contenedor, sin proxy TLS real delante) recibe un 301 hacia `https://`, y como no hay nada escuchando TLS ahí, `urlopen` falla siguiendo la redirección. Fix: el healthcheck manda `X-Forwarded-Proto: https` en el pedido, simulando exactamente lo que nginx manda en tráfico real — no sacar la protección para que el healthcheck pase.
- **Lección operativa: `docker compose restart` de un contenedor recién crasheado no siempre resetea bien el resolver DNS embebido de Docker** — `mcp_server` crasheaba de forma consistente (no una carrera puntual) intentando resolver `openaipublic.blob.core.windows.net` (tiktoken descargando su encoding la primera vez que se usa, sin cache pre-poblado como sí tiene dev vía el bind mount) con `restart`, pero funcionaba perfecto con `docker compose run` (contenedor nuevo) o recreando el contenedor (`rm` + `up`) en vez de reiniciar el existente. Mismo binario, misma red, mismas credenciales — la única diferencia fue recrear vs. reiniciar.
- **Verificado de punta a punta con las 5 credenciales/settings de prueba reales** (no solo `docker compose config` o una lectura del yaml): los 6 servicios (`db`, `redis`, `backend`, `worker`, `mcp_server`, `frontend`) levantan sanos desde una imagen reconstruida desde cero, y el frontend sirve contenido real (`curl` a `/` devuelve 200). La verificación corrió aislada (stack de dev pausado con `stop`, no `down`, y restaurado después) para no volver a pisar el namespace de dev.

**Notas de la implementación real (Tarea 3 — backup/restore):**
- `infra/scripts/backup.sh`/`restore.sh`, no comandos sueltos en un README: `pg_dump -Fc` (formato custom, comprimido, restaurable selectivamente) vía `docker compose exec db`, sin necesitar `pg_dump`/`pg_restore` instalados en el host. `COMPOSE_FILE`/`ENV_FILE` como variables de entorno para apuntar a dev o prod sin duplicar el script.
- **Los vectores de pgvector no necesitan nada especial**: `pg_dump` trata el tipo `vector` como cualquier otro tipo de columna de una extensión — el único requisito es que `CREATE EXTENSION vector` ya haya corrido en el destino antes de restaurar (`postgres/init.sql` ya lo hace en cualquier base nueva).
- `restore.sh` es destructivo a propósito (`--clean --if-exists`, deja el destino idéntico al dump) y pide confirmación explícita antes de tocar nada.
- **Verificado de punta a punta, no solo revisando el script**: backup real contra la base de dev (3.1MB), restaurado en un contenedor de Postgres descartable y aislado (no la base real, para no arriesgar nada) — los conteos post-restore (3 workspaces, 4 documentos, 553 chunks, **los 553 con su embedding intacto**) coinciden exactamente con la base real. `infra/backups/` sumado a `.gitignore`: un dump tiene notas personales reales adentro, nunca debería poder terminar commiteado.

**Notas de la implementación real (Tarea 4 — rate limiting):**
- **El throttling nativo de DRF no aplica**: el endpoint de chat (`rag/chat/views.py`) es una vista Django plana (`@login_required` + `@require_POST`), no un `APIView` — `throttle_classes` es un mecanismo específico de DRF. Se armó un decorador propio (`core/ratelimit.py`), genérico, sobre el mismo cache de Redis que ya usa el proyecto (`CACHES["default"]`).
