...
- **`Chunk.search_vector` sigue calculándose solo sobre `content`, sin `heading_path`** (surgió arreglando el hueco de `heading_path`, 2026-08). Los embeddings y el prompt ya usan `content_with_heading`, pero la mitad **léxica** del hybrid search no: su `GeneratedField` indexa únicamente `content`, así que una búsqueda léxica por una fecha no matchea los chunks partidos de esa misma entrada. Arreglarlo es cambiar la expresión a `SearchVector("heading_path", "content", ...)`, lo que implica una migración sobre una columna generada (se recalcula sola para todas las filas). **Medido después: para la query de fecha real no cambiaría nada** — `LexicalRetriever` devuelve literalmente 0 resultados porque la semántica AND de `websearch_to_tsquery` falla con las palabras en español, no por falta de la fecha. Sigue siendo deuda válida (mejoraría el léxico en general), pero no es la palanca para el caso de fechas.
...
- **Medición real: arreglar `heading_path` NO arregló el bug de fechas. `DateAwareRetriever` sigue siendo necesario.** (2026-08, medido con los 2390 chunks re-embedidos, 0 pendientes). La hipótesis era que los chunks sin fecha en su texto eran la causa raíz; la medición la refutó. Con la query real `"Que hice el 21 de Julio de 2025? (2025-07-21)"` y los 2 chunks objetivo **confirmados embebidos**:
  - `LexicalRetriever` top-50 → **0 resultados**. Cero, no "los equivocados". Confirma de nuevo la semántica AND de `websearch_to_tsquery`: la pregunta en español contra contenido en inglés no matchea nada, y ninguna cantidad de `heading_path` cambia eso (por eso tampoco alcanzaría con meter `heading_path` en `search_vector`: el problema son las palabras españolas, no la fecha).
  - `DenseRetriever` top-50 → los chunks objetivo **no aparecen**. Este es el dato clave: es un fallo de **recall**, no de ranking. El reranker nunca llega a verlos, así que un cross-encoder mejor no lo arreglaría.
  - `HybridRetriever` top-50 y `Hybrid+Rerank` top-5 → tampoco, como consecuencia directa de lo anterior.
  - Con `DateAwareRetriever`: 2 de 5 en el top-5, los correctos, en las dos primeras posiciones.
  - **El fallo más informativo**: sin `DateAwareRetriever`, el resultado #1 es un chunk de **`2022-07-21`** — mes y día correctos, **año equivocado**. El bi-encoder trata la fecha como un patrón de tokens parecido y no discrimina el año. Es evidencia concreta y reproducible de por qué un embedding no sirve para matcheo de valores exactos, y justifica que el parche determinístico exista.
  - Conclusión: `heading_path` se arregló porque estaba genuinamente mal (un dato calculado que nadie leía, y los chunks partidos quedaban sin contexto), pero **su valor es mejorar la calidad general del retrieval, no resolver preguntas por fecha exacta**. Son dos problemas distintos.
